...
impl<'a> NonrepeatingIterator<'a> {
    fn new(
        ...
        NonrepeatingIterator {
            ...
            buf: NumberSet::AlreadyListed(Default::default()),
        }
    }
    ...
    fn produce(&mut self) -> usize {
        ...
        match &mut self.buf {
            NumberSet::AlreadyListed(already_listed) => {
                ...
                if numberset_should_list_remaining(already_listed.len(), range_size) {
...
fn numberset_should_list_remaining(listed_count: usize, range_size: usize) -> bool {
    // Arbitrarily determine the switchover point to be around 25%. This is because:
    // - HashSet has a large space overhead for the hashtable load factor.
...
mod test_numberset_decision {
    use super::numberset_should_list_remaining;

    #[test]
    fn test_stay_positive_large_remaining_first() {
        assert_eq!(false, numberset_should_list_remaining(0, std::usize::MAX));
    }

    #[test]
    fn test_stay_positive_large_remaining_second() {
        assert_eq!(false, numberset_should_list_remaining(1, std::usize::MAX));
    }

    #[test]
    fn test_stay_positive_large_remaining_tenth() {
        assert_eq!(false, numberset_should_list_remaining(9, std::usize::MAX));
    }

    #[test]
    fn test_stay_positive_smallish_range_first() {
        assert_eq!(false, numberset_should_list_remaining(0, 12345));
    }

    #[test]
    fn test_stay_positive_smallish_range_second() {
        assert_eq!(false, numberset_should_list_remaining(1, 12345));
    }

    #[test]
    fn test_stay_positive_smallish_range_tenth() {
        assert_eq!(false, numberset_should_list_remaining(9, 12345));
    }

    #[test]
    fn test_stay_positive_small_range_not_too_early() {
        assert_eq!(false, numberset_should_list_remaining(1, 10));
    }
    ...
    #[test]
    fn test_switch_half() {
        assert_eq!(true, numberset_should_list_remaining(1234, 2468));
    }

    // Ensure that the decision is monotonous:
    #[test]
    fn test_switch_late1() {
        assert_eq!(true, numberset_should_list_remaining(12340, 12345));
    }

    #[test]
    fn test_switch_late2() {
        assert_eq!(true, numberset_should_list_remaining(12344, 12345));
    }

    // Ensure that we are overflow-free:
    #[test]
    fn test_nocrash_exceed_maxsize1() {
        assert_eq!(
            false,
            numberset_should_list_remaining(12345, std::usize::MAX)
        );
    }

    #[test]
    fn test_nocrash_exceed_maxsize2() {
        assert_eq!(
            true,
            numberset_should_list_remaining(std::usize::MAX - 1, std::usize::MAX)
        );
    }

    #[test]
    fn test_nocrash_exceed_maxsize3() {
        assert_eq!(
            true,
            numberset_should_list_remaining(std::usize::MAX, std::usize::MAX)
        );
    }
}
